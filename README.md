...
![AIGuide 首页：蓝色新手入口可选豆包或 WorkBuddy，紫色进阶入口使用 ChatGPT 制作网页](docs/readme/home.jpg)
...
以上为本地生产构建的实际页面截图，**首页更新于 2026-09-16，其余截图保留 2026-09-15 的状态**，可点击查看原图。榜单图片是一份时间明确的快照；课程中的产品账号实测状态以正文标注为准。截图与维护说明见[预览资产说明](docs/readme/README.md)。
...
| 第一次接触 AI | 选择入门工具 → 豆包网页版或 WorkBuddy 桌面版 | 整理待办清单，或读取练习记录生成周报文件 |
| 已经会使用 AI，想尝试编程 | 网络准备 → ChatGPT 桌面版中的 Codex | 创建一个个人网页，再进行修改与验收 |
...
| 工具教程 | 13 家公司、32 个产品与模型入口，按网页版、桌面版、API、模型组织 |
| 使用手册 | OpenAI、WorkBuddy、ZCode、Claude Code 四份本站中文手册，共 40 节；功能用法、进阶技巧、16 条常见问题、案例与练习，保留官方文档入口 |
| 教程实操 | 46 篇教程；网络准备、豆包、WorkBuddy 与 ChatGPT 网页练习提供材料、中文步骤、核对与问题帮助 |
| 我的学习 | 阅读位置、收藏、自查、项目进度和 10 篇成果草稿；JSON 备份与 TXT 导出 |
| 试用反馈 | 四种试用任务、公开内容预览、GitHub Issue 提交与本地留存 |
| 模型排名 | Artificial Analysis 综合能力与终端编程快照，保留来源、指标与日期 |
| 独立阅读 | 46 篇独立 HTML 教程及阅读目录，无需 JavaScript 即可阅读正文与来源 |

“使用手册”提供本站原创整理的步骤、示例任务与检查方法，保留官方跳转和核对范围；四份手册另有同源生成的独立 HTML 阅读版，不计入原有 46 篇课程或自动标记学习完成。

案例区复用两项已有实测产物（Codex 网页修改、WorkBuddy 周报核对），另提供两项可下载教学练习（ZCode 窄屏布局、Claude Code 数量汇总）。练习材料与验收目标不代表已在对应产品账号内执行。
...
当前优先补齐豆包、WorkBuddy 与 ChatGPT 编程入口的账号内实操、新手试用记录及待核对教程。发布反馈前请移除账号、密钥和私人对话。
