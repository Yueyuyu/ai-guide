...
2026-09-15 实际打开豆包网页，确认“对话”、文字输入、发送和登录控件，并用本站虚构通知试填、试发。该次未登录试发后返回首页，未取得回答；原记录及哈希继续保留在 `public/tutorials/doubao-notice/`。

2026-09-18 用户本人登录后完成豆包网页提问、追问、复制和回站下载；已读取磁盘文件核对原通知与完整回答。七张实拍及范围清单位于 `public/tutorials/doubao-live-20260918/`，真实文本位于 `public/practice/doubao-session/`。新增来源 `doubao-practice` 对应修正版原文哈希，不覆盖旧未登录证据。保留“部分步骤待核对”：系统记事本、真实手机、附件与搜索未测，登录过程未录制，底层模型 ID 未确认。
...
| 豆包网页版 | 部分步骤待核对 | [豆包网页版](https://www.doubao.com/chat/) | 2026-09-18 已实测提问、追问、复制、回站下载与磁盘核对；保留真实初稿和修正版。系统记事本、真实手机、附件与搜索未测；AIGuide 成果保存是本站功能。 |
