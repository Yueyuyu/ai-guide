...
豆包与 WorkBuddy 之后，2026-09-16 又精简了网络准备、ChatGPT／Codex 主线和其他软件共 24 篇课。已有章节顺序、记录与草稿入口保持不变。官方截图必须注明来源日期与示例性质，不能用生成图片代替操作证据；本轮新增四张官方原图，其他位置使用明确标注的本站图解，未补齐所有产品的账号内实测截图。

章节可配置 `visual: { title, kind, label, items, note }`。`kind` 支持 `flow`（默认步骤）、`compare`（对照）、`files`（文件关系）、`commands`（命令）；每项包含 `title`、可选的 `text`、`code`、`icon` 和 `logo`。`icon` 使用 tutorial-presentation.js 中的语义符号名；`logo` 使用 brands 下已有的原始品牌文件名，不改绘品牌。只表达教学关系，不绘制假按钮或假运行输出；默认显示“步骤图解 · 非软件截图”。命令逐条复制，认证会话命令必须写明输入位置。
