...
章节可配置 `visual: { title, kind, label, items, note }`。`kind` 支持 `flow`（默认步骤）、`compare`（对照）、`files`（文件关系）、`commands`（命令）；每项包含 `title`、可选的 `text` 和 `code`。只表达教学关系，不绘制假按钮或假运行输出；默认显示“步骤图解 · 非软件截图”。命令逐条复制，认证会话命令必须写明输入位置。
