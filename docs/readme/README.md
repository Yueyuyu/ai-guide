...
五个徽章分别说明工程检查状态、MIT 许可、React、Vite 与欢迎贡献。版本依据 package.json；工程检查使用 GitHub 动态徽章，不写死“通过”。公开试用地址使用单独的在线体验链接；尚无覆盖率报告或正式 Release，不展示相应徽章；社区规模由 GitHub 自身展示。

## 实际截图

首页截图于 2026-09-15 随本次双人群入口与 WorkBuddy 教程更新，来自本地生产构建的 1440 × 1100 可见区域；对应代码与图片在同一次提交中保存。其余截图保留源码提交 `3a57411` 的本地生产构建，使用内置浏览器 1280 × 720 视口；排名为完整页面，其余为可见区域。截图保留原始内容，未添加不存在的功能或替换分数。
