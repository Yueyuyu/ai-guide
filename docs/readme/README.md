...
首页截图于 2026-09-16 随紧凑玻璃入口卡更新，来自本地生产构建的 1440 × 1000 可见区域；对应代码与图片在同一次提交中保存。其余截图保留源码提交 `3a57411` 的本地生产构建，使用内置浏览器 1280 × 720 视口；排名为完整页面，其余为可见区域。截图保留原始内容，未添加不存在的功能或替换分数。
