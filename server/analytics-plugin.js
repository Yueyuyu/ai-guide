export function analyticsMarkup(token = '') {
  ...
  // 统计加载不阻塞页面就绪；只统计整页访问，不把 Hash 切换当作课程完成。
  return `<script type="module" async src="https://static.cloudflareinsights.com/beacon.min.js" data-cf-beacon='${JSON.stringify({ token, spa: false })}'></script>`;
}
