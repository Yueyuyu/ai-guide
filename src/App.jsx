...
export function App() {
  ...
  useEffect(() => {
    const title = route.page === 'learn' ? lessonById[route.id]?.title : route.page === 'company' ? companies.find(company => company.id === route.id)?.name : ['tool', 'model'].includes(route.page) ? tools.find(tool => tool.id === route.id)?.name : route.page === 'path' ? paths.find(path => path.id === route.id)?.title : route.page === 'project' ? projects.find(project => project.id === route.id)?.title : ({ models: '模型指南', model: '模型学习', ranking: '模型排名', search: '搜索教程与工具', discover: '从这里，学会 AI。', tutorials: '全部教程', tools: '工具教程', paths: '学习路线', projects: '实战项目', library: '我的学习' })[route.page];
