...
import { manualById } from './data/manual-catalog.js';
...
const Manuals = lazy(() => import('./pages/Manuals.jsx').then(module => ({ default: module.Manuals })));

export function App() {
  const route = useRoute();
  useEffect(() => {
    const title = route.page === 'learn' ? lessonById[route.id]?.title : route.page === 'company' ? companies.find(company => company.id === route.id)?.name : ['tool', 'model'].includes(route.page) ? tools.find(tool => tool.id === route.id)?.name : route.page === 'path' ? paths.find(path => path.id === route.id)?.title : route.page === 'project' ? projects.find(project => project.id === route.id)?.title : ({ start: '选择入门工具', feedback: '试用与反馈', models: '模型指南', model: '模型学习', ranking: '模型排名', search: '搜索教程与工具', discover: '从这里，学会 AI。', tutorials: '全部教程', tools: '工具教程', paths: '学习路线', projects: '实战项目', library: '我的学习' })[route.page];
    const manualTitle = route.page === 'manuals' ? (route.id ? (Object.hasOwn(manualById, route.id) ? manualById[route.id].title : '手册未找到') : '使用手册') : null;
    document.title = `${manualTitle || (route.page === 'about' ? '关于与来源' : title || '页面未找到')} — AIGuide`;
  }, [route]);
  let content;
  switch (route.page) {
    case 'feedback': content = <Feedback key={route.params.get('track')} route={route}/>; break;
    case 'manuals': content = <Manuals id={route.id} section={route.params.get('section')}/>; break;
    case 'discover': content = <Discover/>; break;
    case 'start': content = <Start/>; break;
