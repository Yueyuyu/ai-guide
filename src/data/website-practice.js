...
export const codexPractice = {
  ...
  sections: [
    { title: '核对条件，再选择 Codex 编程入口', paragraphs: ['官方桌面说明目前提供 ChatGPT 和 Codex 的统一入口。安装并登录后，打开练习文件夹，选择 Codex，用 New chat（新建聊天）开始。本教程中的“网页”是作品，不是 Codex 云端模式。', '先让工具说明当前工作目录，确保它看到的是这次练习文件夹。不要把桌面上的旧项目当作空白目录。服务访问、系统支持和账号权益以官方当前说明为准；仅安装成功不代表已经能执行任务。'], actionSteps: ['在电脑的“文档”中创建 AIGuide-practice 文件夹，再在其中创建空的 personal-page 文件夹。Windows 用文件资源管理器，Mac 用访达。', '从下方官方入口安装支持当前系统的桌面应用，打开后按官网提示登录。', '使用应用中的打开文件夹入口，选择刚创建的 personal-page。选择 Codex，再用 New chat（新建聊天）开始任务。', '先发送：“不要修改文件，只报告当前完整工作目录和已有文件。”核对路径与自己的空目录一致，再继续下一步。'], links: [{ href: 'https://learn.chatgpt.com/docs/app', label: '官方桌面安装与入门', external: true }, { href: 'https://learn.chatgpt.com/docs/windows/windows-app', label: 'Windows 官方说明', external: true }, { href: '#/path/starter', label: '还没准备好？先从网页入门' }], checkpoint: '账号可用，任务返回的完整路径与自己的练习目录一致。' },
    ...
    { title: '保存创建记录，继续修改', paragraphs: ['网页文件留在自己的电脑目录。下面仅保存创建过程、打开方法和待解决问题，不会上传或执行你的网页代码。下一课沿用这个目录和Codex，不需要换编辑器。'], resultEditor: true },
  ],
};

export const websiteLessons = [
  {
    id: 'codex-iterate', productId: 'codex', platform: 'desktop', lessonType: 'operation', title: '继续用 Codex，完成一次明确修改', description: '在同一个网页上提出小范围改动，查看文件差异并复查。', category: 'coding', level: '入门', minutes: 25, tools: ['codex'], color: 'peach', cover: ['让网页再好一点', '说清范围，检查差异'], edited: '2026-09-15',
    ...
    sections: [
      { title: '先留一份修改前的对照', paragraphs: ['在文件管理器里复制一份index.html作为自己的备份，或使用已经配置好的版本控制。重新打开当前页面，记录原来三个项目的样子。', '本课做一个明确的新需求：为三个项目分别加上阅读、计划、记录标签。即使页面本身没有Bug，也能练习有限修改。'] },
  ...
  },
  {
    ...
    sections: [
      ...
      { title: '保存本地交付记录', paragraphs: ['确认index.html和README.md仍在练习目录，运行说明与实际打开方式一致。下面保存验收记录；页面源文件需要自己另行备份。'], resultEditor: true, links: [{ href: '#/project/personal-page', label: '完成个人主页项目自查' }, { href: '#/learn/cursor-debug', label: '选学：用 Cursor 排查表单问题' }] },
