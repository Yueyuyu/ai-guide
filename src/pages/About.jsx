...
export function About() {
  return <div className="about-page"><PageHeading title="先学会使用，再慢慢深入。" description="AIGuide 是一份面向中文读者的 AI 学习手册。"/>
    ...
    <section><h2>内容如何保持可复核</h2><p>教程正文会提示超过30天未复核的来源；榜单快照超过7天会提醒更新。日期按资料实际核对和快照获取计算，不会因为修改文案自动刷新。当前仍需维护者主动更新。</p><p>每篇教程末尾都能生成问题记录，附带教程、章节和你的描述。可以复制或下载后用于复核；目前未接入在线提交，不会自动发送给维护者。</p><a className="text-link" href={import.meta.env.BASE_URL + 'read/index.html'}>打开独立阅读目录<Icon name="arrow" size={16}/></a><p>每篇独立阅读页有自己的网址、标题和摘要，关闭JavaScript也能阅读。互动练习与本地进度继续在互动版使用。</p></section>
