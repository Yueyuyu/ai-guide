...
export function Discover() {
  return <><section className="home-intro"><div className="home-eyebrow"><i/>AI 入门指南</div><h1>从这里，学会 AI。</h1><p>从认识 AI 到熟练使用工具，一步一步学清楚。</p></section>
    <section className="audience-paths" aria-label="按使用基础选择起点">
      <a className="audience-card audience-beginner" href="#/start"><div className="audience-top"><span className="audience-badge">第一次使用 AI</span><span className="audience-note">推荐起点</span></div><h2>从一份小任务开始</h2><p>选豆包或 WorkBuddy，完成第一份成果。</p><div className="audience-footer"><div className="audience-products">{['doubao', 'workbuddy'].map(id => <span key={id}><ToolMark tool={tools.find(tool => tool.id === id)} size="small"/>{id === 'doubao' ? '豆包' : 'WorkBuddy'}</span>)}</div><span className="audience-action">选择工具<Icon name="arrow" size={17}/></span></div></a>
      <a className="audience-card audience-builder" href="#/path/builder"><div className="audience-top"><span className="audience-badge">已有基础</span><span className="audience-note">想做出作品</span></div><h2>用 ChatGPT，做一个网页</h2><p>准备好网络与账号，用 Codex 创建、修改并验收。</p><div className="audience-footer"><div className="audience-products"><span><ToolMark tool={tools.find(tool => tool.id === 'chatgpt-desktop')} size="small"/>ChatGPT 桌面版</span></div><span className="audience-action">开始练习<Icon name="arrow" size={17}/></span></div></a>
    </section>
