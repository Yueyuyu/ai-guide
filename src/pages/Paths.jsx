...
export function Paths() {
  return <><PageHeading title="找到适合你的起点" description="按现在的基础，选一条路。"/><div className="routes-grid">{['starter', 'network', 'builder', 'office', 'developer'].map(id => { const path = paths.find(p => p.id === id); return <article className={'route-card tone-' + path.color} key={id}><CourseArt kind={id === 'starter' ? 'brain' : path.icon}/><div><span className="tag">{id === 'starter' ? '推荐起点' : path.audience}</span><h2>{labels[id]}</h2><p>{path.subtitle}</p><p className="muted">{path.sequence.length} 节课 · {path.outcome}</p><a className={'button ' + (id === 'starter' ? 'primary' : 'secondary')} href={'#/path/' + id}>查看路线<Icon name="arrow"/></a></div></article>; })}</div><div className="soft-panel horizontal-panel"><div><h2>学一点，就做一点。</h2><p>用一个小项目，把学过的东西连起来。</p></div><a className="text-link" href="#/projects">查看实战项目<Icon name="arrow"/></a></div></>;
}
export function PathDetail({ id }) {
  ...
  return <><a className="back-link" href="#/paths">学习路线 / {labels[id]}</a><PageHeading title={path.title} description={path.subtitle}/>
    <div className="route-detail-layout"><div>
      ...
      {['starter', 'builder'].includes(id) && <a className="starter-practice-link" href={learnHref(id === 'starter' ? 'doubao-notice' : 'codex-web', id)}>已有基础？直接进入{ id === 'starter' ? '豆包网页' : 'Codex 桌面' }实操<Icon name="arrow" size={16}/></a>}
      {id === 'builder' && <a className="starter-practice-link" href="#/path/network">官方服务还打不开？先做网络准备<Icon name="arrow" size={16}/></a>}
    ...
    </div><aside className="route-summary"><h2>学完后，你会带走</h2><p>{path.outcome}</p><p>{progress.done} / {progress.total} 课已完成</p><Progress value={progress.percent} label="路线总进度"/><p>{id === 'starter' ? '手机可阅读；网页实操截图以电脑浏览器为例。成果可以下载成 TXT。' : '练习材料随课提供。记录可在“我的成果”中继续编辑和导出。'}</p><a className="text-link" href={id === 'network' ? '#/path/builder' : path.projectId ? '#/project/' + path.projectId : id === 'developer' ? '#/models' : '#/project/research-note'}>{id === 'network' ? '进入 Codex 编程路线' : id === 'developer' ? '选择具体厂商接口' : '进入对应实战项目'}<Icon name="arrow"/></a></aside></div></>;
}
