...
import { manualHref } from '../data/manual-catalog.js';
export function Search({ route }) {
  ...
  return <><PageHeading title={q ? '关于“' + q + '”的学习内容' : '搜索教程与工具'} description="先看内容类型，再进入适合你的页面。"/><SearchBox inputId="global-search" query={q} onSearch={value => { window.location.hash = url({ q: value }); }} placeholder="搜索公司、工具、模型或任务"/>
    <nav className="search-tabs" aria-label="搜索结果类型">{[['all', '全部'], ['lessons', '教程'], ['manuals', '使用手册'], ['products', '工具'], ['models', '模型'], ['paths', '路线']].map(([id, name]) => <a key={id} href={url({ type: id })} aria-current={type === id ? 'page' : undefined}>{name}</a>)}</nav>
    {!q ? <EmptyState title="从一个名字，找到一条学法。" description="试试搜索 Claude、豆包或资料整理。" href="#/path/starter" action="还没想好？从入门路线开始"/> : !total ? <EmptyState title="没有找到匹配的内容" description={'当前关键词：' + q + '。可以换个词，或查看全部教程。'}/> : <>
      ...
      {shown('manuals') && result.manuals.length > 0 && <section className="search-group"><h2>本站使用手册</h2><div className="product-rows outlined">{result.manuals.map(manual => <a key={manual.id} href={manualHref(manual.id)}><Icon name="file" size={28}/><div><strong>{manual.title}</strong><small>{manual.description}</small></div><Icon name="arrow"/></a>)}</div></section>}
